...
- **Monorepo structure** — [`docs/decisions/ADR-005-monorepo-structure-and-tooling.md`](docs/decisions/ADR-005-monorepo-structure-and-tooling.md). pnpm + Turborepo + Changesets. `apps/` + `packages/` split; shared base configs in `packages/config/`; workspace protocol for inter-package deps; independent per-package semver.
